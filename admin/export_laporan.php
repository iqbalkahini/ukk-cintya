...
if ($format === 'excel') {
    ...
    header('Content-Disposition: attachment; filename=laporan-lelang-' . $tanggal_dari . '-sampai-' . $tanggal_sampai . '.xls');
    ?>
    <table border="1">
        <tr>
            <th colspan="7">Laporan Lelang Periode <?php echo htmlspecialchars(formatTanggal($tanggal_dari)); ?> s/d <?php echo htmlspecialchars(formatTanggal($tanggal_sampai)); ?></th>
        </tr>
        <tr>
            <th>No</th>
            <th>Tanggal</th>
...
if ($format === 'pdf') {
    ...
    $pdf = buildSimplePdf($lines, 'Laporan Lelang');
    header('Content-Type: application/pdf');
    header('Content-Disposition: attachment; filename=laporan-lelang-' . $tanggal_dari . '-sampai-' . $tanggal_sampai . '.pdf');
    echo $pdf;
    exit;
}
